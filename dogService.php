...
if ($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_POST['submit'])) {
    ...
    $dog_sex = trim($_POST['dog_sex']);
    $dog_breed = trim($_POST['dog_breed']);
    $dog_age = trim($_POST['dog_age']);
    ...
    $stmt = $conn->prepare("INSERT INTO dog_booking (firstname, lastname, phone, email, address, check_in_date, check_in_time, check_out_date, check_out_time, dog_name, dog_sex, dog_breed, dog_age) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)");

    if (!$stmt) {
        die("Prepare failed: " . $conn->error);
    }

    $stmt->bind_param("sssssssssssss", $firstname, $lastname, $phone, $email, $address, $check_in_date, $check_in_time, $check_out_date, $check_out_time, $dog_name, $dog_sex, $dog_breed, $dog_age);

    if ($stmt->execute()) {
        echo "<script>alert('Booking successful!'); window.location.href='dogService.php';</script>";
    } else {
    ...
    <section id="product1" class="section-p1">
        ...
        <div>
            <ul id="sidebar">  
                ...
                <li><a class="active" href="dogService.php">Dog</a></li>
                <li><a href="hamsterService.php">Hamster</a></li>
            </ul>
        </div>
        <br>
        <div id="banner_service2" class="section-m1"></div>
    </section>
        ...
        <div class="boarding-box">
            <form method="POST" action="">
                <div class="booking-container" id="booking">
                    <div class="top">
                        <header>Dog Boarding Booking Form</header>
                    </div>    
                    <div class="two-forms">
                        <div class="input-box">
                    ...
                    <br>
                    <header>Your Dog's Details</header>
                    <div class="two-forms">
                        <div class="input-box">
                            <input type="text" class="input-field" name="dog_name" placeholder="Name" required>
                            <i class='bx bxs-dog'></i>                    
                        </div>
                        <div class="input-box">
                            <input type="text" class="input-field" name="dog_sex" placeholder="Sex" required>
                            <i class='fa fa-venus-mars'></i>                    
                        </div>
                    </div>
                    <div class="two-forms">
                        <div class="input-box">
                            <input type="text" class="input-field" name="dog_breed" placeholder="Breed" required>
                            <i class='bx bxl-baidu'></i>                    
                        </div>
                        <div class="input-box">
                            <input type="text" class="input-field" name="dog_age" placeholder="Age" required>
