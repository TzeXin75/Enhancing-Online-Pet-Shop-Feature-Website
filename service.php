...
if ($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_POST['submit'])) {
    ...
    $cat_sex = trim($_POST['cat_sex']);
    $cat_breed = trim($_POST['cat_breed']);
    $cat_age = trim($_POST['cat_age']);
    ...
    $stmt = $conn->prepare("INSERT INTO cat_booking (firstname, lastname, phone, email, address, check_in_date, check_in_time, check_out_date, check_out_time, cat_name, cat_sex, cat_breed, cat_age) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)");

    if (!$stmt) {
        die("Prepare failed: " . $conn->error);
    }

    $stmt->bind_param("sssssssssssss", $firstname, $lastname, $phone, $email, $address, $check_in_date, $check_in_time, $check_out_date, $check_out_time, $cat_name, $cat_sex, $cat_breed, $cat_age);

    if ($stmt->execute()) {
        echo "<script>alert('Booking successful!'); window.location.href='service.php';</script>";
    } else {
...
?>
    ...
    <section id="product1" class="section-p1">
        ...
        <div>
            <ul id="sidebar">  
                <li><a class="active" href="service.php">Cat</a></li>
                <li><a href="dogService.php">Dog</a></li>
                <li><a href="hamsterService.php">Hamster</a></li>
            </ul>
        </div>
        <br>
        <div id="banner_service" class="section-m1"></div>
    </section>
        ...
        <div class="boarding-box">
            <form method="POST" action="">
                <div class="booking-container" id="booking">
                    <div class="top">
                        <header>Cat Boarding Booking Form</header>
                    </div>    
                    <div class="two-forms">
                        <div class="input-box">
                    ...
                    <br>
                    <header>Your Cat's Details</header>
                    <div class="two-forms">
                        <div class="input-box">
                            <input type="text" class="input-field" name="cat_name" placeholder="Name" required>
                            <i class='bx bxs-cat'></i>                        
                        </div>
                        <div class="input-box">
                            <input type="text" class="input-field" name="cat_sex" placeholder="Sex" required>
                            <i class='fa fa-venus-mars'></i>                    
                        </div>
                    </div>
                    <div class="two-forms">
                        <div class="input-box">
                            <input type="text" class="input-field" name="cat_breed" placeholder="Breed" required>
                            <i class='bx bxl-baidu'></i>                    
                        </div>
                        <div class="input-box">
                            <input type="text" class="input-field" name="cat_age" placeholder="Age" required>
